...
[ "$1" = "" -o "$1" = "--help" ] && exit_with 1 "\nUsage: `basename $0` [--delete] 3.0|3.1|3.2|4.0-latest[-light]|5-<alpha|beta|latest>[-light]|<4.0|5>-devel [<server image tag(s)> [<platform Git tag (only applicable to 5-latest and 5-latest-light>]]\n"
...
then
	VERSION=4.0
	BRANCH=release
	TAGS=${2:-centos-openjdk-11 centos-temurin-17 centos-temurin-11}
	SRVS=tomcat
	PFTAG=$1
	GITTAG=$3
	if [ "$GITTAG" != "" ]
...
then
	...
	TAGS=${2:-centos-openjdk-11 centos-openjdk-1.8.0 centos-temurin-17 centos-temurin-11 centos-temurin-8}
	SRVS=tomcat
	PFTAG=$1
	GITTAG=$3
	if [ "$GITTAG" != "" ]
...
then
	VERSION=5
	BRANCH=master
	TAGS=${2:-centos-openjdk-11 centos-temurin-17 centos-temurin-17-jre}
	SRVS=tomcat
	PFTAG=$1
...
then
	VERSION=5
	BRANCH=master
	TAGS=${2:-centos8-openjdk-17 centos8-openjdk-11 rockylinux almalinux adoptium-17 adoptium-11 alpine alpine-temurin}
	SRVS=tomcat
	PFTAG=$1
elif [ "$1" = "5-devel" ]
then
...
then
	VERSION=5
	BRANCH=release
	TAGS=${2:-centos-openjdk-11 centos-temurin-17 centos-temurin-17-jre centos-openjdk-11 centos-temurin-11 centos-jvmless}
	SRVS=tomcat
	PFTAG=$1
	GITTAG=$3
	if [ "$GITTAG" != "" ]
...
then
	VERSION=5
	BRANCH=$1
	TAGS=${2:-centos-openjdk-11 centos-temurin-17}
	SRVS=tomcat
	PFTAG=$1
else
	rm -f $LOCK
